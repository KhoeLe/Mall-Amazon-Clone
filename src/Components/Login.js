...
function Login() {
  ...
  const register = (event) =>{
      ...
      auth
        .createUserWithEmailAndPassword(email, password)  // api for firebase
        .then((auth) =>{
          history.push("/login");
  ...
  return (
    <div className="login">
      ...
      <form > 
        ...
        <h5>Password</h5> 
        <input  value={password} onChange={event => setPassword(event.target.value)} type="password" autocomplete="on" />   
